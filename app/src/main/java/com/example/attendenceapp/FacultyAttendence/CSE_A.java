...
public class CSE_A extends AppCompatActivity {
    ...
    private void showPopupMenu(View view) {
        ...
        popupMenu.setOnMenuItemClickListener(item -> {
            if (item.getItemId() == R.id.addStudent) {
                ...
                Toast.makeText(CSE_A.this, "Edit Attendance is Clicked", Toast.LENGTH_SHORT).show();
                return true;
            } else if (item.getItemId() == R.id.print) {
                Toast.makeText(CSE_A.this, "Print", Toast.LENGTH_SHORT).show();
                return true;
            }
            return false;
        });
    ...
    void addStudentDialog() {
        ...
        addStudent.setOnClickListener(v -> {
            ...
            if (!id.isEmpty()) {
                ...
                Toast.makeText(CSE_A.this, "Please enter Student ID", Toast.LENGTH_SHORT).show();
            }
        });
                ...
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        if (!task.getResult().isEmpty()) {
                            Toast.makeText(CSE_A.this, "Student ID " + id + " is already registered", Toast.LENGTH_SHORT).show();
                        } else {
                            addStudentToFirestore(id);
                        }
                    } else {
                        Toast.makeText(CSE_A.this, "Error checking student: " + task.getException(), Toast.LENGTH_SHORT).show();
                    }
                });
    }
    ...
    void addStudentToFirestore(String studentId) {
        ...
        subjectsCollection.add(newStudent).addOnSuccessListener(documentReference -> {
            ...
            Toast.makeText(CSE_A.this, "Student added successfully", Toast.LENGTH_SHORT).show();
            dialog.dismiss();
        }).addOnFailureListener(e -> {
            Toast.makeText(CSE_A.this, "Failed to add student: " + e.getMessage(), Toast.LENGTH_SHORT).show();
        });
    }
    ...
    private void markAttendanceForStudents(String date, String time, String subjectCode) {
        ...
        for (AttendenceModal student : adapterList) {
            ...
            DocumentReference summaryRef = firestore.collection("Attendance_Summary")
                    .document(studentId).collection("attendance").document(subjectCode);

            batch.update(summaryRef, "totalClasses", FieldValue.increment(1));
            if (isPresent) {
                batch.update(summaryRef, "classesPresent", FieldValue.increment(1));
            }

            // Add detailed attendance entry for student view
            DocumentReference detailedAttendanceRef = firestore.collection("Main_Semesters")
                    .document(semester).collection("Sections")
                    .document(section).collection("Subjects")
                    .document(subjectCode).collection("Students")
                    .document(studentId).collection("attendance").document(date);
            ...
            batch.set(detailedAttendanceRef, detailedAttendanceData, SetOptions.merge());
        }

        // Commit batch updates
        batch.commit().addOnSuccessListener(aVoid -> {
            Toast.makeText(CSE_A.this, "Attendance marked successfully!", Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
        }).addOnFailureListener(e -> {
            Toast.makeText(CSE_A.this, "Failed to mark attendance: " + e.getMessage(), Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
        });
    }
